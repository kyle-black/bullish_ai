...
for symbol in symbols:
    ...
    opening_ratio = (
        1 - (third_newest_bar["open"])/(third_newest_bar['close']))
    second_ratio = (
        1 - (second_newest_bar["open"])/(second_newest_bar['close']))
    ratio_compare = second_ratio/opening_ratio
    ratio_compare_final = (newest_bar["open"]/newest_bar['close']-1)

    if (opening_ratio >= .03) and (ratio_compare < .33) and (ratio_compare_final > .02):
