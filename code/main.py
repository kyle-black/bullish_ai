...
def index(request: Request):
    ...
    rows = cursor.fetchall()
    cursor.execute("""
        select symbol, 100.0*(close-open)/open As PercentDiff, close, low, high, close, volume
    ...
    return templates.TemplateResponse('index4.html', {"request": request, "stocks": rows, "indicator_values": indicator_values})
...
def stock_detail(request: Request, symbol):
    ...
    cursor.execute(
        """SELECT * FROM stock_price WHERE stock_id =?ORDER BY date DESC""", (rows['id'],))
...
def strategy(request: Request):
    ...
    cursor.execute("""
    SELECT symbol, name, strategy_id
    FROM stock JOIN stock_strategy ON stock_strategy.stock_id= stock.id
    """)

    main_stocks = cursor.fetchall()
